...
for k=1:TestIter
    ...
    BestSol.PSO = P.PSO(1);
    BestCost(k) = min([BestSol.GA.Cost BestSol.PSO.Cost]);
    nFE =nFE+2*nPop;
end
CostDecrease = abs([BestFormer.Cost-BestSol.GA.Cost BestFormer.Cost-BestSol.PSO.Cost]/BestFormer.Cost);
...
if fnum==8 || fnum==9
 plot(BestCost,'LineWidth',1,'Color',ColorS(9));
else
 semilogy(BestCost,'LineWidth',1,'Color',ColorS(9));
end
xlabel('Iteration');
ylabel('Cost');
grid on;
