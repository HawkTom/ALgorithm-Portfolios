...
VarSize=[1 nVar];   % GAcision Variables Matrix Size
nFE=0;
TestIter=50; Al_str={'HS ','GA '};
%% ALgorithm Initial
Pop=Init(nPop,fnum);
GlobalBest=Pop(1);
...
for k=1:TestIter
    ...
    P.GA = GA(P.GA,fnum);
    BestSol.GA = P.GA(1);
    BestCost(k) = min([BestSol.HS.Cost BestSol.GA.Cost]);
    nFE =nFE+2*nPop;
end
CostGAcrease = abs([BestFormer.Cost-BestSol.HS.Cost BestFormer.Cost-BestSol.GA.Cost]/BestFormer.Cost);
[~,AlFlag] = max(CostGAcrease);
if AlFlag==1
    Pop = P.HS;
else
    Pop = P.GA;
...
if fnum==8 || fnum==9
 plot(BestCost,'LineWidth',1,'Color',ColorS(7));
else
 semilogy(BestCost,'LineWidth',1,'Color',ColorS(7));
end
xlabel('Iteration');
ylabel('Cost');
grid on;
