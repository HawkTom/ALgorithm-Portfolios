...
for k=1:TestIter
    ...
    P.DE = DE(P.DE,fnum);
    BestSol.DE = P.DE(1);
    BestCost(k) = min([BestSol.HS.Cost BestSol.DE.Cost]);
    nFE =nFE+2*nPop;
end
CostDecrease = abs([BestFormer.Cost-BestSol.HS.Cost BestFormer.Cost-BestSol.DE.Cost]/BestFormer.Cost);
...
if fnum==8 || fnum==9
    plot(BestCost,'LineWidth',2,'Color',ColorS(6));
else
    semilogy(BestCost,'LineWidth',2,'Color',ColorS(6));
end
xlabel('Iteration');
ylabel('Cost');
grid on;
