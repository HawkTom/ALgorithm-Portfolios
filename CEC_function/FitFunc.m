function [ fit_r ] = FitFunc(id,u,num)
...
for i=1:u
    ...
    switch num
        ...
        case 10 %Ackley's Function
            fit_r(i) = -20*exp(-0.2*rms(x))-exp(rms(cos(2*pi*x)))+20+2.71828;
        ...
        case 13 %Generalized Penalized Function
            ...
            pos = x>-10*ones(n,1) & x<10*ones(n,1);
            temp(pos)=0;  
            fit_r(i) = 0.1*(sin(3*pi*x(1)).^2 + sum((x(1:n-1)-1).^2.*(1+sin(3*pi*x(2:n)).^2)) + (x(n)-1)*(1+sin(2*pi*x(n)).^2))+sum(temp);
        ...
        case 18 % Goldstein-Price Function
            n=2;
            fit_r(i)=(((x(1)+x(2)+1)^2)*(19-14*x(1)+3*(x(1)^2)-14*x(2)+6*x(1)*x(2)+3*(x(2)^2))+1)*(30+((2*x(1)-3*x(2))^2)*(18-32*x(1)+12*(x(1)^2)+48*x(2)-36*x(2)*x(1)+27*(x(2)^2)));
        case 19 %Hartman's Family
            n=4;
            a=[3 10 30;0.1 10 35;3 10 30;0.1 10 35]';
            c=[1 1.2 3 3.2];
            p=[0.3689 0.1170 0.2673;0.4699 0.4387 0.7470;0.1090 0.8732 0.5547;0.03815 0.5742 0.8828]';
            x=repmat(x,1,4);
