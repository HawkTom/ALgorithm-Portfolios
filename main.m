...
for k=1:TestIter
    ...
    BestSol.PSO = P.PSO(1);
    BestCost(k) = min([BestSol.HS.Cost BestSol.PSO.Cost]);
    nFE =nFE+2*nPop;
end
CostDecrease = abs([BestFormer.Cost-BestSol.HS.Cost BestFormer.Cost-BestSol.PSO.Cost]/BestFormer.Cost);
...
if fnum==8 || fnum==9
    plot(BestCost,'LineWidth',2,'Color',ColorS(10));
else
    semilogy(BestCost,'LineWidth',2,'Color',ColorS(10));
end
xlabel('Iteration');
ylabel('Cost');
grid on;
